...
func DefaultP2PConfig() *P2PConfig {
	return &P2PConfig{
		ListenAddress:    "tcp://0.0.0.0:46655",
		NodeKeyFile:      "nodekey",
		SkipUPNP:         false,
		LANDiscover:      true,
		MaxNumPeers:      50,
...
func DefaultDataDir() string {
	...
	home := homeDir()
	if home == "" {
		return "./.anonymitycash"
	}
	switch runtime.GOOS {
	case "darwin":
		// In order to be compatible with old data path,
		// copy the data from the old path to the new path
		oldPath := filepath.Join(home, "Library", "Anonymitycash")
		newPath := filepath.Join(home, "Library", "Application Support", "Anonymitycash")
		...
		return newPath
	case "windows":
		return filepath.Join(home, "AppData", "Roaming", "Anonymitycash")
	default:
		return filepath.Join(home, ".anonymitycash")
	}
}

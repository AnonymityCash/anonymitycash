...
import (
	...
	"github.com/anonymitycash/anonymitycash/p2p/upnp"
)
...
func getUPNPExternalAddress(externalPort, internalPort int) (*NetAddress, error) {
	...
	externalPort, err = nat.AddPortMapping("tcp", externalPort, internalPort, "anonymitycashd tcp", 0)
	if err != nil {
		return nil, errors.Wrap(err, "could not add tcp UPNP port mapping")
	}
	externalPort, err = nat.AddPortMapping("udp", externalPort, internalPort, "anonymitycashd udp", 0)
	if err != nil {
...
// DefaultListener Implements anonymitycashd server Listener

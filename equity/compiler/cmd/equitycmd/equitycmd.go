...
import (
	...
	"github.com/anonymitycash/anonymitycash/equity/compiler"
)

var (
	// generateInstPath is the directory (need to combine with GOPATH) for store generated contract instance
	generateInstPath = "/src/github.com/anonymitycash/anonymitycash/equity/instance/"
)
...
func main() {
	...
	imports := map[string]bool{
		...
		"encoding/hex": true,
		"fmt":          true,
		"github.com/anonymitycash/anonymitycash/equity/compiler": true,
		"github.com/anonymitycash/anonymitycash/protocol/vm":     true,
	}
...
func asGoParams(params []*compiler.Param) (goParams string, imports []string) {
	...
	for _, p := range params {
		...
		case "Asset":
			typ = "bc.AssetID"
			imports = append(imports, "github.com/anonymitycash/anonymitycash/protocol/bc")
			strFlag = true
		case "Boolean":
			typ = "bool"
		case "Hash":
		...
		case "PublicKey":
			...
			imports = append(imports, "github.com/anonymitycash/anonymitycash/crypto/ed25519")
			strFlag = true
		case "Signature":
			typ = "[]byte"
			strFlag = true
	...
	if strFlag {
		imports = append(imports, "github.com/anonymitycash/anonymitycash/encoding/json")
	}
